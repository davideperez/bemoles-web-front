...
const EventDetail = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && router.query.id && router.query.id !== 'add') {
      (async () => {
  ...
  return (
    <>
     <Head>
        <title>{!event ? 'Cargando...': `${router.query.id === 'add' ? 'Crear evento' : `Editar evento: ${event.title}`}}`} | Los bemoles</title>
      </Head>
    ...
    <Skeleton isLoaded={!!event || router.query.id === 'add'}>
      ...
      <Flex alignItems="center" mb={2}>
          ...
          />
        <Heading>{router.query.id === 'add' ? 'Crear evento' : `Editar evento: ${event?.title}`}</Heading>
        </Flex>
              ...
              <form onSubmit={handleSubmit}>
                <Stack spacing={2}>
                  ...
                  <FormControl>
                    ...
                    />
                    <Image
                      src={image ? URL.createObjectURL(image) : values.flyer}
                      alt={values.title}
                      boxSize="150px"
                      mt={4}
                      border="1px solid"
